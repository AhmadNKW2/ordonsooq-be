import { Controller, Get, Post, Body, Patch, Param, Delete, Query, UseGuards, Req, BadRequestException, UseInterceptors, UploadedFile } from '@nestjs/common';
...
export class BrandsController {
    ...
    async create(
        ...
        if (logo) {
            const uploadResult = await this.r2StorageService.uploadFile(logo, 'brands');
    ...
    @UseGuards(AuthGuard('jwt'), RolesGuard)
    @Roles(UserRole.ADMIN)
    permanentDelete(@Param('id') id: string, @Body() options?: PermanentDeleteBrandDto) {
        return this.brandsService.permanentDelete(+id, options);
    }

    @Delete(':id')
    removeFallback() {
        throw new BadRequestException('Use /brands/:id/permanent for deleting brands');
    }
}
