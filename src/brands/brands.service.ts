...
import { RestoreBrandDto, PermanentDeleteBrandDto } from './dto/archive-brand.dto';
...
export class BrandsService {
    ...
    async create(dto: CreateBrandDto, logoUrl?: string): Promise<Brand> {
        const existing = await this.brandsRepository.findOne({ where: [{ name_en: dto.name_en }, { name_ar: dto.name_ar }] });
        if (existing) {
    ...
    async findAll(filterDto?: FilterBrandDto) {
        const { page = 1, limit = 10, sortBy = 'sort_order', sortOrder = 'ASC', status = BrandStatus.ACTIVE, visible, search } = filterDto || {};
    ...
    async update(id: number, dto: UpdateBrandDto, logoUrl?: string): Promise<Brand> {
        ...
        if (logoUrl && oldLogoUrl) {
            try {
                ...
                this.logger.warn(`Failed to delete old brand logo: ${oldLogoUrl}`, error);
            }
        }
    ...
    private async syncProductsToBrand(brandId: number, product_ids: number[]): Promise<void> {
        // Remove this brand from existing products
        await this.productsRepository.update({ brand_id: brandId }, { brand_id: null as any });
    ...
    async restore(id: number, restoreDto?: RestoreBrandDto): Promise<{ message: string; restoredProducts: number; skippedProducts: number; skippedReason?: string; }> {
        const brand = await this.brandsRepository.findOne({
    ...
    async permanentDelete(id: number, options?: PermanentDeleteBrandDto): Promise<{ message: string }> {
        const brand = await this.brandsRepository.findOne({
            where: { id, status: BrandStatus.ARCHIVED },
        });

        if (!brand) {
            throw new NotFoundException('Brand not found or not archived. Only archived brands can be permanently deleted.');
        }
        ...
        if (archivedProductCount > 0) {
            ...
            if (options.moveProductsToBrandId) {
                const targetBrand = await this.brandsRepository.findOne({
                    where: { id: options.moveProductsToBrandId, status: BrandStatus.ACTIVE },
                });

                if (!targetBrand) {
                    throw new BadRequestException('Target brand not found or is archived');
                }
                ...
                await this.productsRepository.delete({ brand_id: id, status: ProductStatus.ARCHIVED });
            }
        }
