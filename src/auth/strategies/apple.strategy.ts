...
export class AppleStrategy extends PassportStrategy(Strategy, 'apple', 6) {
  constructor(private configService: ConfigService) {
    ...
    const privateKeyLocation = configService.get<string>(
    ...
    if (!privateKeyString && !privateKeyLocation) {
      throw new Error(
        'Either APPLE_PRIVATE_KEY or APPLE_PRIVATE_KEY_LOCATION must be set',
      );
    }
    ...
    const finalPrivateKey = privateKeyString
      ? formatKey(privateKeyString)
      : undefined;
  ...
  async validate(
    ...
    if (!appleId) {
      console.error(
        '🍎 ❌ CRITICAL: Could not extract Apple ID from ANY source!',
      );
      console.error(
        '   Full debug:',
        JSON.stringify(
