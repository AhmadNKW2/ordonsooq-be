...
import { RestoreCategoryDto, PermanentDeleteCategoryDto, RestoreSubcategoryOptions, RestoreProductsOptions } from './dto/archive-category.dto';
...
export class CategoriesService {
     ...
     */
    private async syncProductsToCategory(categoryId: number, product_ids: number[]): Promise<void> {
    ...
    async findAll(filterDto?: FilterCategoryDto) {
        const { page = 1, limit = 10, sortBy = 'sortOrder', sortOrder = 'ASC', visible, status, parent_id, level, search } = filterDto || {};

        const queryBuilder = this.categoriesRepository
            .createQueryBuilder('category')
            .leftJoinAndSelect('category.parent', 'parent')
            .leftJoinAndSelect('category.children', 'children')
            .where('category.status = :activeStatus', { activeStatus: CategoryStatus.ACTIVE }); // Only active categories
    ...
    async getCategoryTree(): Promise<Category[]> {
        const mainCategories = await this.categoriesRepository
            .createQueryBuilder('category')
            .leftJoinAndSelect('category.children', 'children', 'children.status = :status', { status: CategoryStatus.ACTIVE })
            .leftJoinAndSelect('children.children', 'grandchildren', 'grandchildren.status = :status', { status: CategoryStatus.ACTIVE })
     ...
     */
    async archive(id: number, userId: number): Promise<{ 
        ...
        const product_ids = [...new Set(productCategories.map(pc => pc.product_id))];
     ...
     */
    async restore(id: number, restoreDto: RestoreCategoryDto): Promise<{
        ...
        if (category.parent_id && !restoreDto.new_parent_id && !restoreDto.makeRoot) {
            const parent = await this.categoriesRepository.findOne({
        ...
        if (restoreDto.restoreAllSubcategories) {
            ...
            const descendantResult = await this.restoreAllDescendants(id, category.level);
        ...
        } else if (restoreDto.subcategories && restoreDto.subcategories.length > 0) {
            // Restore specific subcategories with their options
            for (const subcatOptions of restoreDto.subcategories) {
                const subcatResult = await this.restoreSubcategory(subcatOptions, category.level + 1);
    ...
    private async restoreProductsForCategory(
        ...
        if (!options || (!options.restoreAll && (!options.product_ids || options.product_ids.length === 0))) {
        ...
        if (options.product_ids && options.product_ids.length > 0) {
            product_ids = product_ids.filter(id => options.product_ids!.includes(id));
        }
    ...
    private async restoreSubcategory(
        ...
        const productResult = await this.restoreProductsForCategory(options.id, options.products);
        ...
        if (options.restoreAllSubcategories) {
            const descendantResult = await this.restoreAllDescendants(options.id, expectedLevel);
            totalRestoredCategories += descendantResult.restoredCategories;
            totalRestoredProducts += descendantResult.restoredProducts;
            totalSkippedProducts += descendantResult.skippedProducts;
            categoryDetail.subcategoriesRestored = descendantResult.restoredCategories;
        } else if (options.subcategories && options.subcategories.length > 0) {
            for (const nestedOptions of options.subcategories) {
                const nestedResult = await this.restoreSubcategory(nestedOptions, expectedLevel + 1);
    ...
    private async restoreAllDescendants(
        ...
        for (const child of children) {
            ...
            const productResult = await this.restoreProductsForCategory(child.id, { restoreAll: true });
            restoredProducts += productResult.restored;
            skippedProducts += productResult.skipped;

            // Recursively restore grandchildren
            const descendantResult = await this.restoreAllDescendants(child.id, parentLevel + 1);
    ...
    async findArchived(filterDto?: FilterCategoryDto) {
        const { page = 1, limit = 10, sortBy = 'archived_at', sortOrder = 'DESC', search } = filterDto || {};
        ...
        const dataWithRelations = await Promise.all(
            data.map(async (cat) => {
                ...
                const archivedSubcategories = await this.categoriesRepository.find({
                    ...
                    select: ['id', 'name_en', 'name_ar', 'image', 'archived_at', 'archived_by'],
                });
     ...
     */
    async permanentDelete(id: number, options?: PermanentDeleteCategoryDto): Promise<{ message: string }> {
        const category = await this.categoriesRepository.findOne({
        ...
        if (!category) {
            throw new NotFoundException('Category not found or not archived. Only archived categories can be permanently deleted.');
        }
        ...
        if (archivedProductCount > 0) {
            ...
            if (options.move_products_to_category_id) {
                ...
                const targetCategory = await this.categoriesRepository.findOne({
                    where: { id: options.move_products_to_category_id, status: CategoryStatus.ACTIVE },
                });

                if (!targetCategory) {
                    throw new BadRequestException('Target category not found or is archived');
                }
                ...
                await this.productsRepository.delete({ category_id: id, status: ProductStatus.ARCHIVED });
            }
        }
     ...
     * Reorder categories
     */
    async reorder(categories: { id: number; sortOrder: number }[]): Promise<{ message: string }> {
        ...
        return { message: `${categories.length} categories reordered successfully` };
    }
     ...
     */
    async getProducts(categoryId: number): Promise<{ category: Category; products: Product[] }> {
        const category = await this.categoriesRepository.findOne({
            where: { id: categoryId },
            relations: ['parent'],
        });
        ...
        const productCategories = await this.productCategoriesRepository.find({
            where: { category_id: categoryId },
            relations: ['product', 'product.vendor', 'product.media', 'product.priceGroups'],
        });
